...
function g2!(storage,vc)
    ...
    DV[:,:,:]=repeat(mean(DV,dims=[1,2]),outer=[nx,ny,1]);
    mat"""
    $DV(:,:,:)=imgaussfilt3($DV,$fu);
    """
    storage[:]=DV[:];
...
g2!,vc,LBFGS(m=5,alphaguess=LineSearches.InitialQuadratic(α0=sca*50.0,αmin=sca*10.0),
...
g2!,vc,LBFGS(m=5,alphaguess=LineSearches.InitialQuadratic(α0=sca*50.0,αmin=sca*10.0),
linesearch=LineSearches.BackTracking(c_1=10.0^(-20))),
Optim.Options(iterations=2,store_trace=true,show_trace=true,
x_tol=0,g_tol=0));
vc=opt1.minimizer;
